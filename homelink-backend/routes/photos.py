...
def upload_photo(annonce_id):
    ...
    if nb_photos >= MAX_PHOTOS_PAR_ANNONCE:
        return jsonify({
            'message': f'Maximum {MAX_PHOTOS_PAR_ANNONCE} photos par annonce'
        }), 400
    ...
    if fichier.content_type not in MIMES_AUTORISES:
        return jsonify({
            'message': 'Format non accepté. Utilisez JPG, PNG ou WebP'
        }), 400
    ...
    try:
        ...
        return jsonify({
            'message': f'Erreur upload Cloudinary : {str(e)}'
        }), 500
...
def get_photos(annonce_id):
    liste = (
        Photo.query
        .filter_by(annonce_id=annonce_id)
        .order_by(Photo.id)
        .all()
    )
